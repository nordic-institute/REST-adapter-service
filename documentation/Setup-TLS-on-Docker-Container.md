...
When the REST Adapter Service communicates over HTTPS, the target server’s TLS certificate must be trusted. In the consumer gateway role, this means importing the Security Server’s internal TLS certificate into the REST Adapter Service’s Java trust store. In the provider gateway role, it means importing the REST service’s TLS certificate.

### Adapter acting as a client for TLS 

Short: Providing Java truststore including the server certificate

You need to get the server certificate (e.g. from your Security Server administrator) and import it into a Java truststore. You can use this command:
...
Short: Providing Java truststore including the server certificate and Java keystore including the adapter key/certificate

For mutual TLS (mTLS) authentication, both the client and server must present valid certificates to each other. Therefore, in addition to providing the target server's certificate in the Java truststore, the adapter's certificate must be added to the Security Server and a keystore needs to be provided on the adapter. 
...
You can do so by adding the volume to your `docker run` command and extending the `JAVA_TOOL_OPTIONS` environment variable like this:
